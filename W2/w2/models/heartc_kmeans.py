...
## Run K-means in reduced Heart-C with PCA
...
ax.set_title('K-means clustering in reduced Heart-C using PCA', fontsize=22)
plt.savefig(os.path.join('..', '..', 'figures', 'heart-c', 'pca-reduced-heart-c_k-means.png'))
plt.show()

file_path = os.path.join('..', '..', 'validation', 'heart-c_k-means_val.txt')
with open(file_path, 'a') as f:
    f.write(f'\n \nK-means: K = {K}, init = {init_method}, metric = {metric} max_inter = {n_iter}, n_init = {init}, reduction=PCA({df.shape[1]})')

utils.print_metrics(df, df_gs, labels, file_path)
print("Finished K-means in reduced Heart-C with PCA")

## Run K-means in reduced Heart-C with UMAP
...
ax.set_title('K-means clustering in reduced Heart-C using UMAP', fontsize=22)
plt.savefig(os.path.join('..', '..', 'figures', 'heart-c', 'umap-reduced-heart-c_k-means.png'))
plt.show()

file_path = os.path.join('..', '..', 'validation', 'heart-c_k-means_val.txt')
with open(file_path, 'a') as f:
    f.write(f'\n \nK-means: K = {K}, init = {init_method}, metric = {metric} max_inter = {n_iter}, n_init = {init}, reduction=UMAP({df.shape[1]})')

utils.print_metrics(df, df_gs, labels, file_path)
print("Finished K-means in reduced Heart-C with UMAP")
