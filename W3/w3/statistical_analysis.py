...
ibl_eval.ff, ibl_eval.crit_val, ibl_eval.which_diff, ibl_eval.crit_dist = k_ibl_utils.friedman_nemenyi(folds_acc.astype(float), alpha=0.1)

alg_acc = np.array([line.split(':')[1].strip(' \n') for line in lines if line.startswith('Mean accuracy')][2:])
