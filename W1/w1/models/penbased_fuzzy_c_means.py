...
print("Starting Fuzzy C-means in Pen-Based")
fuzzy = FuzzyCMeans(C, max_iter=12)
fuzzy.fit(df)
...
plt.savefig(os.path.join('..', '..', 'figures', 'pen-based', f'penbased_fuzzy-c-means-{C}.png'))
plt.show()
...
plt.savefig(os.path.join('..', '..', 'figures', 'pen-based', f'penbased_fuzzy-c-means-MF-{C}.png'))
plt.show()

file_path = os.path.join('..', '..', 'validation', 'penbased-fuzzy_val.txt')
with open(file_path, 'a') as f:
    f.write(f'\n\nFuzzy C-Means: C={C}')
    f.write(f'\nPartition coefficient: {partition_coefficient(fuzzy.u_matrix, df.shape[0])}\n')

utils.print_metrics(df, df_gs, labels, file_path)
print("Finished Fuzzy C-means in Pen-Based")
